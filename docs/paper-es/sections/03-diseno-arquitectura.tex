...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Clase de Modelo}: Típicamente usa \texttt{model\_class='frankensteindecoder'} para arquitecturas de decoder autorregresivo.
\end{itemize}
...
La conexión residual no es solo una elección de modelado, sino también una decisión de macro-diseño. Además del residual de identidad estándar, este sistema implementa Hyper-Connexiones Restringidas por Variedad (mHC) \citep{xie_mhc_2025}, un reemplazo opcional controlado por el sub-objeto \texttt{model.mhc} (clave plana \texttt{use\_mhc}, por defecto \texttt{false}). mHC expande el ancho del flujo residual por un factor \texttt{expansion\_rate} $n$ (el flujo pasa a ser $n \times \texttt{hidden\_size}$ mientras los internos de la capa permanecen en \texttt{hidden\_size}) y restringe la matriz de mezcla de flujo $H^{\text{res}}$ al politopo de Birkhoff---el conjunto de matrices doblemente estocásticas---mediante una proyección Sinkhorn--Knopp diferenciable (\texttt{sinkhorn\_iters} $= 20$ por defecto). Esto restaura la propiedad de mapeo de identidad que pierden las Hyper-Connexions sin restricción: el producto residual compuesto permanece doblemente estocástico a profundidad arbitraria, la norma espectral satisface $\|H^{\text{res}}\|_2 \le 1$ y $H^{\text{res}} x$ actúa como una combinación convexa de las características del flujo. Los escalares de compuerta $\alpha$ se inicializan pequeños (\texttt{gating\_init} $= 0.01$), y \texttt{expansion\_rate} $= 1$ degenera al mapeo de identidad exacto. mHC es mutuamente excluyente con el enrutamiento de tokens Mixture-of-Depths y mantiene su proyección de coeficientes en precisión completa bajo BitNet por defecto (\texttt{full\_prec\_under\_bitnet}). La formulación completa, el algoritmo Sinkhorn--Knopp, el cableado de implementación y los resultados reportados se detallan en el Apéndice~\ref{sec:annex-mhc}.
