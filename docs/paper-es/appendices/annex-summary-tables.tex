...
La Tabla~\ref{tab:all-residuals} compara los esquemas de conexión residual disponibles en el código base: el residual de identidad estándar y el flujo residual de $n$ streams restringido por variedad (mHC) \citep{xie_mhc_2025}. $n$ es el factor de expansión del flujo, $C$ la dimensión oculta, $t_{\max}$ el número de iteraciones Sinkhorn--Knopp y $\alpha$ los escalares de compuerta. HC (Hyper-Connexions sin restricción) se lista como referencia pero \emph{no} está expuesto en este código base.
...
\endfoot
\bottomrule
\caption{Esquemas de conexión residual. $x_l \in \mathbb{R}^{C}$ (estándar) o $\mathbb{R}^{n \times C}$ (HC/mHC) es el flujo en la capa $l$, $F$ la función de capa y $H^{\text{pre}}, H^{\text{post}}, H^{\text{res}}$ los mapeos aprendidos de lectura/escritura/mezcla. mHC se controla con los campos de esquema \texttt{model.mhc}; los detalles completos están en el Anexo~9.}
