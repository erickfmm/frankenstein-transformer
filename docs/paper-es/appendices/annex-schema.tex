...
\begin{itemize}[nosep]
    ...
    \item \texttt{model\_class} (enum) --- Variante de arquitectura del modelo: \texttt{frankenstein} (codificador de arquitectura mixta) o \texttt{frankensteindecoder} (decodificador causal autorregresivo). Obligatorio cuando \texttt{base\_model} no está establecido.
...
dims.retention\_heads & int $\ge 1$ & Número de cabezas para capas RetNet. Debe dividir \texttt{hidden\_size} exactamente. \\
dims.dropout & float [0,1] & Tasa de dropout global. Típico: 0.1 (estándar BERT). \\
dims.layer\_pattern & array of enum (obligatorio) & Secuencia ordenada de tipos de capa. La longitud debe igualar \texttt{num\_layers}. Más de 36 tipos de mezclador disponibles (ver Tabla~\ref{tab:layer-pattern}). \\
\midrule
\multicolumn{3}{l}{\textit{model.norm.*}} \\
\midrule
norm.type & enum & Estrategia de normalización: \texttt{layer\_norm}, \texttt{dynamic\_tanh}, \texttt{derf}, \texttt{rms\_norm}, \texttt{prms\_norm}. \\
...
\midrule
attention.titan.positional\_encoding & enum (hope, rope) & Método de codificación posicional para capas \texttt{titan\_attn}. \\
attention.titan.use\_hope & bool & Bandera heredada para HoPE en \texttt{titan\_attn}. Obsoleta; preferir \texttt{positional\_encoding}. \\
attention.titan.hope.base & float $\ge 0$ & Escala de frecuencia base para HoPE. Típico: 10000. \\
attention.titan.hope.damping & float $\ge 0$ & Coeficiente de amortiguación para decaimiento de atención HoPE. Típico: 0.01. \\
attention.titan.rope.base & float $\ge 0$ & Frecuencia base para RoPE. Típico: 10000. \\
attention.titan.rope.scaling & float $\ge 0$ & Multiplicador para índices de posición RoPE. Típico: 1.0. \\
\midrule
...
\midrule
\multicolumn{3}{l}{\textit{model.* (nivel superior plano)}} \\
\midrule
...
use\_bitnet\_conv & bool & Cuantizar Conv1d de embedding con BitConv1d. Requiere \texttt{use\_bitnet=true} y \texttt{embedding.conv.enabled=true}. Por defecto: \texttt{false}. \\
...
ffn\_activation\_config & object & Parámetros anidados para activaciones aprendibles (grados/versión/init de RAF, init PReLU, etc.). \\
...
Los campos se agrupan bajo sub-objetos jerárquicos (\texttt{dims}, \texttt{norm}, \texttt{embedding.factorized}, \texttt{embedding.conv}, \texttt{attention.titan}, \texttt{attention.mla}, \texttt{attention.gqla}, etc.); cada fila de sub-encabezado marca el prefijo vigente para las filas que le siguen. Los campos listados bajo \texttt{model.*} permanecen como claves planas de nivel superior.

\subsubsection{Enumeración del Patrón de Capas}
\label{tab:layer-pattern}

El array \texttt{model.dims.layer\_pattern} acepta los siguientes 36 tipos de mezclador. Cada elemento define una capa física en la pila del modelo. La Tabla~\ref{tab:layer-pattern-enum} enumera todos los tipos disponibles con su soporte de entrenamiento e inferencia.
...
\endlastfoot
task & enum (mlm, sbert, causal\_lm) (obligatorio) & Objetivo de entrenamiento. \texttt{mlm} = Modelado de Lenguaje Enmascarado; \texttt{sbert} = ajuste fino Sentence-BERT; \texttt{causal\_lm} = predicción autorregresiva del siguiente token (requiere \texttt{model\_class='frankensteindecoder'}). \\
...
\begin{enumerate}[nosep]
    ...
    \item Cuando \texttt{base\_model} está configurado, \texttt{training.task} es obligatorio. Si \texttt{task=mlm}, \texttt{tokenizer.name\_or\_path} también es obligatorio --- salvo cuando \texttt{tokenizer.source=train\_from\_dataset}, en cuyo caso el tokenizador se entrena desde el corpus y no se necesita nombre.
    \item \texttt{tokenizer.source=train\_from\_dataset} requiere el sub-objeto \texttt{tokenizer.training} (con \texttt{vocab\_size} para \texttt{bpe}/\texttt{wordpiece}/\texttt{wordlevel}) y es mutuamente excluyente con \texttt{tokenizer.name\_or\_path}. El vocabulario entrenado sobrescribe \texttt{model.dims.vocab\_size} antes de construir el modelo.
    \item \texttt{task: mlm} requiere \texttt{training.optimizer}; \texttt{task: sbert} requiere \texttt{training.sbert}; \texttt{task: causal\_lm} requiere \texttt{training.optimizer} y \texttt{model\_class='frankensteindecoder'}.
    ...
    \item \texttt{model\_class: frankensteindecoder} fuerza \texttt{model.dims.mode: decoder} en tiempo de ejecución. Establecer \texttt{model.dims.mode: encoder} con esta clase se sobrescribe.
