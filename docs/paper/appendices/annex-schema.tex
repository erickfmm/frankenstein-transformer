...
The Frankenstein Transformer toolkit enforces a strict YAML configuration schema with \texttt{additionalProperties: false} at all levels. The schema is defined in \texttt{src/schema.yaml}, which references modular sub-schemas in \texttt{src/schema/}. Every configurable field must be declared in the schema; unrecognized keys are rejected at validation time. This appendix documents the complete schema structure, including all fields, their types, constraints, and cross-component validation rules.
...
\begin{itemize}[nosep]
    ...
    \item \texttt{model\_class} (enum) --- Model architecture variant: \texttt{frankenstein} (mixed-architecture encoder) or \texttt{frankensteindecoder} (autoregressive causal decoder). Required when \texttt{base\_model} is not set.
...
dims.retention\_heads & int $\ge 1$ & Number of heads for RetNet layers. Must divide \texttt{hidden\_size} evenly. \\
dims.dropout & float [0,1] & Global dropout rate. Typical: 0.1 (BERT standard). \\
dims.layer\_pattern & array of enum (required) & Ordered sequence of layer types. Length must equal \texttt{num\_layers}. 35+ mixer types available (see Table~\ref{tab:layer-pattern}). \\
\midrule
\multicolumn{3}{l}{\textit{model.norm.*}} \\
\midrule
norm.type & enum & Normalization strategy: \texttt{layer\_norm}, \texttt{dynamic\_tanh}, \texttt{derf}, \texttt{rms\_norm}, \texttt{prms\_norm}. \\
...
\midrule
\multicolumn{3}{l}{\textit{model.attention.titan.*}} \\
\midrule
attention.titan.positional\_encoding & enum (hope, rope) & Positional encoding method for \texttt{titan\_attn} layers. \\
attention.titan.use\_hope & bool & Legacy flag for HoPE in \texttt{titan\_attn}. Deprecated; prefer \texttt{positional\_encoding}. \\
attention.titan.hope.base & float $\ge 0$ & Base frequency scale for HoPE. Typical: 10000. \\
attention.titan.hope.damping & float $\ge 0$ & Damping coefficient for HoPE attention decay. Typical: 0.01. \\
attention.titan.rope.base & float $\ge 0$ & Base frequency for RoPE. Typical: 10000. \\
attention.titan.rope.scaling & float $\ge 0$ & Multiplier for RoPE position indices. Typical: 1.0. \\
\midrule
...
\midrule
\multicolumn{3}{l}{\textit{model.* (flat top-level)}} \\
\midrule
...
use\_bitnet\_conv & bool & Quantize embedding Conv1d with BitConv1d. Requires \texttt{use\_bitnet=true} and \texttt{embedding.conv.enabled=true}. Default: \texttt{false}. \\
...
ffn\_activation\_config & object & Nested parameters for learnable activations (RAF degrees/version/init, PReLU init, etc.). \\
...
Fields are grouped under hierarchical sub-objects (\texttt{dims}, \texttt{norm}, \texttt{embedding.factorized}, \texttt{embedding.conv}, \texttt{attention.titan}, \texttt{attention.mla}, \texttt{attention.gqla}, etc.); each sub-header row marks the prefix in effect for the rows that follow it. Fields listed under \texttt{model.*} remain flat top-level keys.

\subsubsection{Layer Pattern Enumeration}
\label{tab:layer-pattern}

The \texttt{model.dims.layer\_pattern} array accepts the following 36 mixer types. Each element defines one physical layer in the model stack. Table~\ref{tab:layer-pattern-enum} lists all available types with their training and inference support.
...
\endlastfoot
task & enum (mlm, sbert, causal\_lm) (required) & Training objective. \texttt{mlm} = Masked Language Modeling; \texttt{sbert} = Sentence-BERT fine-tuning; \texttt{causal\_lm} = autoregressive next-token prediction (requires \texttt{model\_class='frankensteindecoder'}). \\
...
\begin{enumerate}[nosep]
    ...
    \item When \texttt{base\_model} is set, \texttt{training.task} is required. If \texttt{task=mlm}, \texttt{tokenizer.name\_or\_path} is also required --- unless \texttt{tokenizer.source=train\_from\_dataset}, in which case the tokenizer is trained from the corpus and no name is needed.
    \item \texttt{tokenizer.source=train\_from\_dataset} requires the \texttt{tokenizer.training} sub-object (with \texttt{vocab\_size} for \texttt{bpe}/\texttt{wordpiece}/\texttt{wordlevel}) and is mutually exclusive with \texttt{tokenizer.name\_or\_path}. The trained vocabulary overrides \texttt{model.dims.vocab\_size} before model construction.
    \item \texttt{task: mlm} requires \texttt{training.optimizer}; \texttt{task: sbert} requires \texttt{training.sbert}; \texttt{task: causal\_lm} requires \texttt{training.optimizer} and \texttt{model\_class='frankensteindecoder'}.
    \item \texttt{bitnet\_routers=true} requires \texttt{use\_bitnet=true}. The flag has no effect otherwise.
    \item \texttt{use\_mhc=true} (mHC n-stream residual) and \texttt{use\_mixture\_of\_depths=true} are mutually exclusive. Enabling both raises a \texttt{ValueError}: MoD token routing operates on a single $C$-dimensional stream, which conflicts with the n-stream residual.
    ...
    \item \texttt{model\_class: frankensteindecoder} forces \texttt{model.dims.mode: decoder} at runtime. Setting \texttt{model.dims.mode: encoder} with this class is overridden.
