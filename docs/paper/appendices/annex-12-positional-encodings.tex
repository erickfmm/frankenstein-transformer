...
\[
\texttt{positional\_encoding} \in \{\texttt{rope},\ \texttt{hope},\ \texttt{nope},\ \texttt{alibi},\ \texttt{pape},\ \texttt{pape\_efficient},\ \texttt{pape\_ri},\ \texttt{sinusoidal\_absolute},\ \texttt{sinusoidal\_rotary},\ \texttt{learned\_absolute},\ \texttt{none}\}
\]
...
\texttt{deltanet\_attn}, \texttt{mtla\_attn}). When \texttt{use\_pe} is
...
removed --- all 11 values now work with every mixer.

\subsection{Architectural Profile: Positional Encodings}

Table~\ref{tab:all-pos-enc} summarises the 11 supported positional
encodings.
