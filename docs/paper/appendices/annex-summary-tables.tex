...
Table~\ref{tab:all-embeddings} catalogs the positional encodings and structural embedding optimizations supported by the codebase. The 11 positional encodings are configured via the model-wide \texttt{positional\_encoding} field (see Annex~12 for full formulations); the two structural optimizations are controlled by their own flags.
...
\endfoot
\bottomrule
\caption{Embedding variants. $V$ = vocabulary size, $H$ = hidden size, $R$ = factorized intermediate dimension. The first 11 rows are positional encodings (Annex~12); the last two are structural embedding optimizations.}
\label{tab:all-embeddings}
\endlastfoot
\multicolumn{3}{c}{\textbf{Positional Encodings (11)}} \\
\midrule
