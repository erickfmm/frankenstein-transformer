...
Table~\ref{tab:all-residuals} compares the residual-connection schemes available in the codebase: the standard identity residual and the manifold-constrained n-stream residual (mHC) \citep{xie_mhc_2025}. $n$ is the stream expansion factor, $C$ the hidden dimension, $t_{\max}$ the Sinkhorn--Knopp iteration count, and $\alpha$ the gating scalars. HC (unconstrained Hyper-Connections) is listed for reference but is \emph{not} exposed in this codebase.
...
\endfoot
\bottomrule
\caption{Residual-connection schemes. $x_l \in \mathbb{R}^{C}$ (standard) or $\mathbb{R}^{n \times C}$ (HC/mHC) is the stream at layer $l$, $F$ the layer function, $H^{\text{pre}}, H^{\text{post}}, H^{\text{res}}$ the learned read/write/mix mappings. mHC is controlled by the \texttt{model.mhc} schema fields; full details in Annex~9.}
