...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Model Class}: Typically uses \texttt{model\_class='frankensteindecoder'} for autoregressive decoder architectures.
\end{itemize}
...
The residual connection is not only a modeling choice but also a macro-design decision. Besides the standard identity residual, this system implements Manifold-Constrained Hyper-Connections (mHC) \citep{xie_mhc_2025}, an opt-in replacement controlled by the \texttt{model.mhc} sub-object (\texttt{use\_mhc} flat key, default \texttt{false}). mHC expands the residual stream width by a factor \texttt{expansion\_rate} $n$ (the stream becomes $n \times \texttt{hidden\_size}$ while layer internals stay at \texttt{hidden\_size}) and constrains the stream-mixing matrix $H^{\text{res}}$ to the Birkhoff polytope---the set of doubly stochastic matrices---via a differentiable Sinkhorn--Knopp projection (\texttt{sinkhorn\_iters} $= 20$ by default). This restores the identity-mapping property lost by unconstrained Hyper-Connections: the composite residual product stays doubly stochastic at arbitrary depth, the spectral norm satisfies $\|H^{\text{res}}\|_2 \le 1$, and $H^{\text{res}} x$ acts as a convex combination of stream features. The gating scalars $\alpha$ are initialized small (\texttt{gating\_init} $= 0.01$), and \texttt{expansion\_rate} $= 1$ degenerates to the exact identity mapping. mHC is mutually exclusive with Mixture-of-Depths token routing and keeps its coefficient projection full-precision under BitNet by default (\texttt{full\_prec\_under\_bitnet}). The full formulation, the Sinkhorn--Knopp algorithm, the implementation wiring, and the reported results are detailed in Appendix~\ref{sec:annex-mhc}.
