...
The gated and fast-weight mechanism coverage is strong across major categories (GLA, DeltaNet, Gated DeltaNet, Gated DeltaNet-2, HGRN2, FoX, Gated Softmax, and the six Falcon variants \citep{zhang_falcon_2026}). Residual-stream macro-design is covered through the manifold-constrained n-stream residual (mHC) \citep{xie_mhc_2025}, whose Sinkhorn--Knopp projection onto the Birkhoff polytope restores the identity-mapping property lost by unconstrained Hyper-Connections; mHC is validated only at the level of the published 3B--27B MoE experiments, and the framework would benefit from independent reproductions at smaller scales (see Appendix~\ref{sec:annex-mhc}).
