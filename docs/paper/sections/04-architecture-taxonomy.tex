...
This system implements forty-two sequence mixer variants organized into seven functional categories reflecting research trends in sequence modeling design. The taxonomical organization reflects evolving understanding of how to balance expressivity, computational efficiency, and memory constraints.
...
% Root
\node[box, fill=blue!15, minimum width=4.0cm] (root) {\textbf{Sequence Mixer Registry (42 variants)}};
...
\draw[line] (f5) -- (f6);
\end{tikzpicture}}
\caption{Comprehensive taxonomy of forty-two sequence mixer variants across seven functional categories. Dense baselines provide full global routing at quadratic cost. Recurrent architectures enable constant-time inference through state compression. Sparse variants reduce complexity through structured patterns or token selection. Gated mechanisms introduce data-dependent control over memory retention and forgetting. Latent methods compress key--value representations into low-rank bottlenecks or route through probabilistic Gaussian-mixture latents. Conditional memory provides test-time read/write over an external memory bank. Fast-weight methods treat the recurrent state transition as an online continual-learning rule.}
