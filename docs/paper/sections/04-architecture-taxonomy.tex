...
This system implements forty-three sequence mixer variants organized into eight functional categories reflecting research trends in sequence modeling design. The taxonomical organization reflects evolving understanding of how to balance expressivity, computational efficiency, and memory constraints.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Latent and Low-Rank Compression} (10): MLA, GQLA, MLRA, Tucker, IHA, GTA, MTLA, CCA, CCGQA, and GMA compress key--value representations into low-rank latent bottlenecks (or, for GMA, route through a learned Gaussian-mixture latent memory), trading a small reconstruction error for substantial memory and compute savings.
    ...
    \item \textbf{Fast-Weight Attention} (6): Falcon-1, Falcon-2, Falcon-3, Falcon-1A, Falcon-2A, and Falcon-3A \citep{zhang_falcon_2026} treat the recurrent fast-weight state transition as an online continual-learning rule under read-after-write semantics, with normalized step sizes, explicit ridge forgetting, and optional bounded sliding-window rehearsal.
    \item \textbf{Geometric Field} (1): SSOG \citep{pisoni_ssog_2026} applies a structured geometric/sparsity field over the attention matrix, encoding domain-specific spatial or graph-structured inductive biases that the other seven families do not capture.
\end{enumerate}
...
% Root
\node[box, fill=blue!15, minimum width=4.0cm] (root) {\textbf{Sequence Mixer Registry (43 variants)}};

% Category headers — 8 columns
...
\node[box, fill=cyan!12, below right=1.0cm and 4.5cm of root, minimum width=2.0cm] (latent) {\textbf{Latent (10)}};
\node[box, fill=teal!12, below right=1.0cm and 7.5cm of root, minimum width=2.0cm] (memory) {\textbf{Memory (1)}};
\node[box, fill=violet!12, below right=1.0cm and 10.5cm of root, minimum width=2.0cm] (fweight) {\textbf{Fast-Weight (6)}};
\node[box, fill=gray!18, below right=1.0cm and 13.5cm of root, minimum width=2.0cm] (geometric) {\textbf{Geometric (1)}};
...
\node[box, fill=cyan!6, below=of l9] (l10) {\texttt{gma\_attn}};

% Memory column (1)
\node[box, fill=orange!6, below=of memory] (m1) {\texttt{engram\_attn}};

% Fast-weight column (6)
\node[box, fill=violet!6, below=of fweight] (f1) {\texttt{falcon1\_attn}};
\node[box, fill=violet!6, below=of f1] (f2) {\texttt{falcon2\_attn}};
\node[box, fill=violet!6, below=of f2] (f3) {\texttt{falcon3\_attn}};
\node[box, fill=violet!6, below=of f3] (f4) {\texttt{falcon1a\_attn}};
\node[box, fill=violet!6, below=of f4] (f5) {\texttt{falcon2a\_attn}};
\node[box, fill=violet!6, below=of f5] (f6) {\texttt{falcon3a\_attn}};

% Geometric column (1)
\node[box, fill=gray!8, below=of geometric] (geo1) {\texttt{ssog\_attn}};
...
\draw[line] (root) -- (fweight);
\draw[line] (root) -- (geometric);
...
% Category-to-list links — Fast-weight
\draw[line] (fweight) -- (f1);
\draw[line] (f1) -- (f2);
\draw[line] (f2) -- (f3);
\draw[line] (f3) -- (f4);
\draw[line] (f4) -- (f5);
...
% Category-to-list links — Geometric
\draw[line] (geometric) -- (geo1);
\end{tikzpicture}}
\caption{Comprehensive taxonomy of forty-three sequence mixer variants across eight functional categories. Dense baselines provide full global routing at quadratic cost. Recurrent architectures enable constant-time inference through state compression. Sparse variants reduce complexity through structured patterns or token selection. Gated mechanisms introduce data-dependent control over memory retention and forgetting. Latent methods compress key--value representations into low-rank bottlenecks or route through probabilistic Gaussian-mixture latents. Conditional memory provides test-time read/write over an external memory bank. Fast-weight methods treat the recurrent state transition as an online continual-learning rule. Geometric-field methods encode spatial or graph-structured inductive biases over the attention matrix.}
...
A complementary family compresses the per-token key--value state into a low-rank latent: Multi-Head Latent Attention (MLA), Group-Query Latent Attention (GQLA), Multi-Head Low-Rank Attention (MLRA), Tucker Attention, Interleaved Head Attention (IHA), Grouped-head laTenT Attention (GTA), and Multi-head Temporal Latent Attention (MTLA). These methods unify grouped-query, latent, factorised, and temporal-latent designs under a common low-rank bottleneck. Gaussian Mixture Attention (GMA) takes a different route: it replaces the pairwise $QK^\top$ interaction with probabilistic routing through $K$ learned Gaussian-mixture components per head, writing values into a $K$-slot latent memory and reading back via posterior responsibilities, yielding $\mathcal{O}(NK)$ activation storage for fixed $K$. Full formulations, pseudocode, and a comparison table are collected in Appendix~\ref{sec:annex-latent}.
...
\end{tikzpicture}}
\caption{Latent and low-rank attention concept. Key--value pairs are compressed through a down-projection into a low-rank latent space ($r \ll d$), where attention is computed at reduced cost, then up-projected back. Variants differ in factorization structure (MLA, GQLA, MLRA, Tucker), head interleaving (IHA, GTA), temporal compression (MTLA), cross-covariance formulations (CCA, CCGQA), and probabilistic Gaussian-mixture routing (GMA).}
\label{fig:latent-map}
\end{figure}

\subsection{Gated Attention Extensions}
The mixer registry includes eight gated blocks: \texttt{gla\_attn}, \texttt{deltanet\_attn}, \texttt{gated\_deltanet\_attn}, \texttt{gated\_deltanet2\_attn}, \texttt{hgrn2\_attn}, \texttt{fox\_attn}, \texttt{gated\_softmax\_attn}, and \texttt{kda\_attn} \citep{yang_gla_2023,yang_deltanet_2024,yang_gated_deltanet_2024,hatamizadeh_gated_deltanet2_2026,qin_hgrn2_2024,lin_forgetting_transformer_2025,qiu_gated_attention_2025,kimi_kda_2025}. The unifying idea is that gating controls \emph{what information survives}: some gates act on recurrent state updates (GLA, DeltaNet variants, HGRN2), while others modify the full attention path itself (FoX and Gated Softmax). KDA extends Gated DeltaNet with a finer-grained, channel-wise gating (decay) mechanism on the delta-rule state update $S_t = (I - \beta_t k_t k_t^\top)D_t S_{t-1} + \beta_t k_t v_t^\top$, where $D_t = \mathrm{Diag}(d_t)$ applies a per-channel decay. This makes gating especially useful when the model must trade off recall, recency, and bounded memory. Mathematical formulations, algorithmic pseudocode, a generic gating template, and a comparison table are collected in Appendix~\ref{sec:annex-gated}.

A sixth family recasts the recurrent fast-weight update itself as an online continual-learning rule: the mixer registry includes six fast-weight blocks --- \texttt{falcon1\_attn}, \texttt{falcon2\_attn}, \texttt{falcon3\_attn}, \texttt{falcon1a\_attn}, \texttt{falcon2a\_attn}, and \texttt{falcon3a\_attn} \citep{zhang_falcon_2026}. Falcon unifies DeltaNet-style regression and linear-attention-style additive writes under read-after-write (prefix-aligned) semantics with normalized step sizes $\eta_t = \beta_t/(\text{statistic}_t + \lambda_t + \varepsilon)$, an objective-derived carry $\gamma_t = 1 - \eta_t\lambda_t$, and bounded sliding-window rehearsal in the Falcon-3 variants. Every variant ships a recurrent reference scan and an SSD-style chunk-parallel kernel, sharing one configuration block. Mathematical formulations, algorithmic pseudocode, the positive-decay renormalization, configuration knobs, and published results are collected in Appendix~\ref{sec:annex-fast-weight}.
